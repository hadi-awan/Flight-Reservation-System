/*
 * Flight System for one single day at YYZ
 */
...
public class FlightReservationSystem {
    public static void main(String[] args) {
        ...
        while (scanner.hasNextLine()) {
            ...
            // For example: res UA267 Hadi DD1234 7B
            // A first class seat is designated by a row, one of ABCD and a ‘+’ character (e.g. 4A+)
                ...
                // If the flight is full, or if the flight does not exist,
                // or if the passenger already exists, or if a seat is already occupied,
                // then return various exceptions accordingly
                String flight;
                String name;
                String passport;
                String seat;
            ...
            // Cancel a reservation on flight for passenger with name and passport
                ...
                // If the user inputs "CANCEL", and supplies a flight-number, name, and passport-number, then cancel a flight for a passenger
                // If the passenger does not already exist, then return a FlightNotFoundException or a PassengerNotManifestException accordingly
                String flight;
                String name;
                String passport;
                if (commandLine.hasNext()) {
            ...
            // Prints out the seats on this flight (example input: seats AC220)
                ...
                // If the user inputs "SEATS", then print out all the seats on the flight
                // If the flight does not exist, then return a FlightNotFoundException
                String flight = null;
                if (commandLine.hasNext()) {
                ...
                // If the user inputs "manifest", then print out the passenger manifest for this flight
                // If the flight does not exist, then return a FlightNotFoundException
                String flight = null;
                if (commandLine.hasNext()) {
                    flight = commandLine.next().toUpperCase();
